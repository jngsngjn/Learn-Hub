    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const basicResponse = await axios.get(`/students/curriculum/${id}/basic`, config);
                ...
                <div className="student_h-left">
                    <h1 className="student_h-logo">
                        <a className="student_navbar-brand" href="/">
                            <img src="/images/logo/HomeLearn_Student_Header_Logo.png" alt="로고"/>
                        </a>
                    </h1>
                    <span className="student_h-curriculum_name">{curriculum.name} {curriculum.th}기</span>
                </div>

                <div className="student_h-right">
                    <div className="student_h-progress-bar">
                        <div className="student_h-progress" style={{width: `${curriculum.progress}%`}}></div>
                    </div>
                    ...
                    <ul className="student_h-gnb_items">
                        ...
                        <div className="student_h-alarm" ref={alarmRef}>
                            <div className="student_h-alarm_btn" onClick={(e) => toggleDropdown('student_alarm', e)}>
                                <li>
                            ...
                            <ul id="student_h-alarm_list"
                                className={`student_h-alarm_list ${openDropdown === 'student_alarm' ? 'open' : ''}`}>
                    ...
                    <div className="student_h-profile_box" ref={profileRef}>
                        <div className={`student_h-profile ${openDropdown === 'student_profile' ? 'active' : ''}`}
                             onClick={(e) => toggleDropdown('student_profile', e)}>
                            <div>
                                <img className="student_h-profile_img" src="/" alt="프로필"/>
                            </div>
                            <span className="student_h-profile_name">{student.name}대성진</span>
                            <i className="fa-solid fa-caret-down"></i>
                        </div>
                        <ul className={`student_h-profile_menu ${openDropdown === 'student_profile' ? 'open' : ''}`}>
